...
namespace DeliveryService.HostedService
{
    public class EventConsumerHostedService : BackgroundService
    {
        ...
        public EventConsumerHostedService(ILogger<EventConsumerHostedService> logger, IServiceProvider serviceProvider)
        {
            _logger = logger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogWarning(ex, "Event Consumer Hosted Service could not start. Retrying in 5 seconds...");
