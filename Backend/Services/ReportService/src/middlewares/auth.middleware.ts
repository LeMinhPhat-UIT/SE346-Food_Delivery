...
export const attachMerchantContext = (req: Request, _res: Response, next: NextFunction) => {
  try {
    if (!req.auth) {
      throw new ApiError(HTTP_STATUS.UNAUTHORIZED, "Authentication required");
    }
    ...
    if (!req.auth.roles.includes(ROLES.ADMIN)) {
      if (!req.auth.roles.includes(ROLES.MERCHANT)) {
        throw new ApiError(HTTP_STATUS.FORBIDDEN, "Merchant role is required for this action");
      }
    }

    req.auth.merchantId = req.auth.merchantId ?? req.auth.userId;
    next();
  } catch (error) {
    next(error);
  }
...
export const attachShipperContext = (req: Request, _res: Response, next: NextFunction) => {
  try {
    if (!req.auth) {
      throw new ApiError(HTTP_STATUS.UNAUTHORIZED, "Authentication required");
    }
    ...
    if (!req.auth.roles.includes(ROLES.ADMIN)) {
      if (!req.auth.roles.includes(ROLES.SHIPPER)) {
        throw new ApiError(HTTP_STATUS.FORBIDDEN, "Shipper role is required for this action");
      }
    }

    req.auth.shipperId = req.auth.shipperId ?? req.auth.userId;
    next();
  } catch (error) {
    next(error);
  }
